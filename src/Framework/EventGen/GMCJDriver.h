...
#include "Framework/Numerical/SampleStats.h"
...
public :
 ...
 ~GMCJDriver();
  ...
  /// neutrino used in the last generated event
  double PathLengthWeightedTotalXSec(void) const
    { return fCurPathLengthWeightedTotalXSec; }

  /// Returns the path-length-weighted total cross section for a flux neutrino
  /// with the given PDG code, 4-momentum, and starting 4-position
  double PathLengthWeightedTotalXSec( int nu_pdg,
    const TLorentzVector& nu_mom4, const TLorentzVector& nu_pos4 ) const;

  /// Returns a running estimate of the flux-averaged total cross section.
  /// The value is based on the events generated by this object so far.
  double FluxAvgTotXSec(void) const;

  /// Returns an estimator for the Monte Carlo statistical uncertainty on the
  /// flux-averaged total cross section returned by FluxAvgTotXSec()
  double FluxAvgTotXSecError(void) const;
...
private:
  ...
  /// Stores a running value (with statistical uncertainty) of the inverse of
  /// the flux-averaged total cross section, including weighting by neutrino
  /// path lengths and the upstream beam simulation
  SampleStats     fInvFluxAvgXSecStats;
};
